...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['AGRICONNECT_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['AGRICONNECT_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Agriconnect API.
 */
export class Agriconnect {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['AGRICONNECT_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('AGRICONNECT_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.AgriconnectError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Agriconnect client with an apiKey option, like new Agriconnect({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Agriconnect.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('AGRICONNECT_LOG'), "process.env['AGRICONNECT_LOG']", this) ??
      defaultLogLevel;
  ...
  static Agriconnect = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static AgriconnectError = Errors.AgriconnectError;
...
Agriconnect.Pets = Pets;
Agriconnect.Store = Store;
Agriconnect.Users = Users;

export declare namespace Agriconnect {
